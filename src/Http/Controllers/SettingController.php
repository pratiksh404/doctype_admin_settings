...
class SettingController extends Controller
{
    ...
    public function uploadImage($setting)
    {
        if (request()->has('string_value') && $setting->setting_type == "Image") {
            ...
            $image = Image::make(request()->file('string_value')->getRealPath())->fit(json_decode($setting->setting_custom)->image->fit->width ?? 800, json_decode($setting->setting_custom)->image->fit->height ?? 800);
            $image->save(public_path('storage/' . $setting->string_value), json_decode($setting->setting_custom)->image->quality ?? 80);
        }
    }
}
